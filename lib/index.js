...
Invoice.prototype.getPlan = function() {
  var interval = this.user._stripe.last_invoice.lines.subscriptions[0].plan.interval
    , planName = this.user._stripe.last_invoice.lines.subscriptions[0].plan.name
    ;
...
Invoice.prototype.render = function(callback) {
  ...
  jade.renderFile(this.jade_path, {
    ...
    layout   : false,
    data     : self.data,
    webappUrl: 'http://www.storify.com' // hard coded here
  }, function(e,html) {
    if (e) return callback(e);
    self.html = html;
    callback(null, html);
...
Invoice.prototype.jade2pdf = function(callback) {
  ...
  var path_file = this.data.transactionId + new Date()
    , path_file = __dirname + '/' + crypto.createHash('md5').update(path_file).digest("hex") + '.pdf';
  self.path_file = path_file;

  port++;
  if (port > 22222) port = 11111;
